...
xhttp.onreadystatechange = function() {
  if (this.readyState == 4 && this.status == 200) {
    ...
    for(var i =0;i < hospi_list.length;i++){
      ...
      if(hospi_list[i].HOSPTL_CENTER_NM.toLowerCase().includes(search) ||
      english_name[i].toLowerCase().includes(search) ||
      hospi_address[i].toLowerCase().includes(search) ||
      hospi_list[i].REPRSNT_TELNO.toLowerCase().includes(search)){
        count++;
    ...
    $("h5").text(count+' search results for keyword "'+sessionStorage.getItem("search")+'"')
  }
};
